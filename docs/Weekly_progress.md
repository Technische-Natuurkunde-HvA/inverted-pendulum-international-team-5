...
The practical consequence was a pronounced vibration of the motor, caused by the two PIDs continuously correcting each other. The system oscillated between contradictory objectives, resulting in a high-frequency switching of the command signal and making the pendulum unstable.

To mitigate this effect, we carried out extensive tuning of the speed PID. We systematically tested a large number of gain combinations (Kp, Ki, Kd), progressively reducing the impact of the speed loop so that it would act as a small damping term rather than a competing controller. Although time-consuming, this process allowed us to observe clear signs of improvement: the motor vibrations decreased, the control action became smoother, and the overall interaction between the two loops became more predictable.

However, due to time constraints, we were not able to continue refining the speed PID as far as necessary. The preliminary results remain encouraging, but additional work is required to properly balance both controllers and avoid structural conflicts between them.

Despite these limitations, the investigations carried out this week significantly improved our understanding of nested control loops and highlighted the need for a more robust coordination mechanism, such as loop decoupling, hierarchical prioritization, or state-based switching, for future iterations.
...
![PWM,RPM = f(time)](Visuals/PWM_RPM_vs_time.png)
...
![RPM vs PWM](Visuals/RPM_vs_PWM.png)
...
in amsterdam side they were able to create the code : 
...
```
and then to save the data we used python 
